...
argumentP :: P.ParsecT [Char] u Identity [Char]         
argumentP = do 
                result <- many1 (noneOf " )")
                many space
                return result
...
actualAction = try parseBaltag <|> try parseAddWeight <|> try parseInform <|> try parseSuspect <|> 
               try parsePrintStatement <|> try parseSequence <|> try parsePublic <|> try parseIfElse <|> 
               try parseIf <|> try parsePrecondition <|> try parseInit <|> try parseUnSet <|> parseSet
...
parsePrintStatement = do 
              symbol "print"
              items <- parens $ parseOutput `sepBy` (symbol ",")
              return $ OstariPrint items
              
parseOutput = try parseString <|> identifier
...
predoptable = [ [ Infix parseAndP AssocLeft ]
          ...
          , [ Prefix parseStrongBelieve ]
          , [ Prefix parseWeightedProbably ]
          , [ Prefix parseProbably ]
          , [ Prefix parseBelieve ]
          , [ Prefix parseNotBelieve ]
          , [ Prefix parseQuantifier ]
          ]
          
parseStrongBelieve = do
                   try (symbol1 "B!") <|> try (symbol1 "[]!")
                   actors <- parens identifier
                   symbol ":"
                   return $ StrongBelieves actors
...
parseProbOperator = do
                      op <- try (symbol "<") <|> try (symbol ">")
                      value <- floating3 True
                      return (if op == "<" then AtMost else AtLeast,value)
                   
parseProbably = do
                   try (symbol1 "P")
                   (op,prob) <- brackets parseProbOperator
                   many space
                   actors <- parens identifier
                   symbol ":"
                   return $ Probably op prob actors
                   
parseWeightProbOperator = do
                            w <- identifier
                            symbol ","
                            (op,prob) <- parseProbOperator
                            return (w,op,prob)
                   
parseWeightedProbably = do
                   try (symbol1 "W")
                   (w,op,prob) <- brackets parseWeightProbOperator
                   many space
                   actors <- parens identifier
                   symbol ":"
                   return $ WeightedProbably w op prob actors
...
toArgs :: [(String,String,[String])] -> AbstractAction String -> AbstractAction String
